...
(function() {
    ...
    module.directive('custom', ['$interval', '$timeout', function($interval, $timeout) {
        ...
        fadeLib.fn.prototype._fade = function(mode, opacity, duration, callback) {
            ...
            var notifyInterval = $interval(func, interval);
//            $interval(func, interval);
        ...
        return {
            ...
            link: function(scope, element, attrs) {
                ...
                var offset = 75 * scope.$index;
                console.log(scope.notification.position);
                element.css(scope.notification.position, offset + 'px')
                var el = fadeLib(element);
                scope.notification.el = el;
                el.fadeIn(200, function() {
                    element.addClass('ngn-active');
                    if (!scope.notification.sticky) {
                        $timeout(function(){
                            el.fadeOut(500, function() {
                                var i = scope.notifications.indexOf(scope.notification);
                                if(i != -1) {
                                    scope.notifications.splice(i, 1);
                                }
                                });
                        }, scope.notification.duration);
                    }
                });
            }
        }
 ...
 /* making a template for the notifications container*/
         $templateCache.put('templates/ng-notify/ngNotify.html',
             '<custom offset="$index * 75" ng-repeat="notification in notifications" class="ngn" ng-class="notification.notifyClass">' +
//                     ' style="%%position%%:{{$index * 75}}px">' +

                /* we could pass the notification it self (or index) to the dismiss function */
                 '<span class="ngn-dismiss" ng-click="dismiss(notification)">&times;</span>' +
     ...
     module.provider('ngNotify', function() {
        ...
        this.$get = ['$document', '$compile', '$rootScope', '$timeout', '$templateCache',
            ...
            function($document, $compile, $rootScope, $timeout, $templateCache) {
                 ...
                 */
                notifyScope.dismiss = function(notification) {
                    notification.el.fadeOut(500, function() {
                        // Find and remove item from an array
                        var i = notifyScope.notifications.indexOf(notification);
                        if(i != -1) {
                            notifyScope.notifications.splice(i, 1);
                        }
                    });
                };
...
//                        $timeout.cancel(notifyTimeout);
                        ...
                        var notification = {
                            ...
                            duration : duration,
                            sticky : sticky,
                            position : userOpts.position || options.position
                        };
